		...
		try (Session session = driver.session()) {
			String response = session.writeTransaction(new TransactionWork<String>() {
				...
				public String execute(Transaction tx) {
					...
					StatementResult actor = tx
							.run(String.format("MATCH (n:actor)" + "WHERE n.actorId = '%s' RETURN n.name", actorId));
					StatementResult movie = tx
							.run(String.format("MATCH (n:movie)" + " WHERE n.movieId = '%s' RETURN n.name", movieId));
					StatementResult exists = tx.run(String.format(
							"RETURN EXISTS( (:actor {actorId: '%s'})-[:ACTED_IN]-(:movie {moveId: %s}) ) AS a", actorId,
							movieId));
